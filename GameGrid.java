...
public class GameGrid {
   ...
   private int Score;// the thickness used for the grid boundaries
   ...
   public void drawExtra() {
	      ...
	      double gridW = gridWidth + gridWidth / 3.0;
	      double startX = gridW - (gridW / 3.0) / 2.0;
	      double startY = gridHeight - (gridHeight / 3.0) / 2.0;
	      StdDraw.setPenColor(wordColor);
	      
	      Font font = new Font("Arial", Font.PLAIN, 25);
	      StdDraw.setFont(font);
	      StdDraw.setPenColor(wordColor);	      
	      StdDraw.text(startX, startY, "Score");	      
	      StdDraw.text(startX, startY - 1, String.valueOf(Score));
	      StdDraw.text(startX, startY - 10, "Next");
	      StdDraw.text(startX, startY - 11, "Tetromino");
	      StdDraw.text(startX, startY - 12, String.valueOf(nextTetromino.getName()));
	   }
   ...
   public void clearFullRows() {
	    ...
	    for (int i = 18 - 1; i >= 0; i--) {
	        ...
	        // If row is full, delete row
	        if (rowIsFull) {
	        	
	        	
	        	// empty the full row
	            for (int j = 0; j < 12; j++) {
	            	Score += tileMatrix[i][j].getNumber();
	                tileMatrix[i][j] = null;
	            }
	        	
	            // Move all lines above the deleted line one line down
	            for (int k = i; k < 18; k++) {
	                for (int j = 0; j < 12; j++) {
	                	if (k == 17) {
	                		continue;
	                	}
	                    tileMatrix[k][j] = tileMatrix[k+1][j];
   ...
   public void gatherSameBox() {
		...
		for (int j = 0; j < 12; j++) {
		    for (int i = 18 - 1; i >= 0; i--) {
		    	...
		    	else if(tileMatrix[i-1][j] == null) {
		    		continue;
		    	}
		    	else if(tileMatrix[i][j].getNumber() == tileMatrix[i-1][j].getNumber()) {
		        	tileMatrix[i][j] = null;
		        	tileMatrix[i-1][j].setNumber(tileMatrix[i-1][j].getNumber()*2);
		        	Score += tileMatrix[i-1][j].getNumber()/2;
		        	break;
		        }
		    }
		}
		for (int j = 0; j < 12; j++) {
		    for (int i = 18 - 1; i >= 0; i--) {
		    	if(tileMatrix[i][j] != null) {
		    		if(i+1<18 && i-1>=0 && j+1<12 && j-1>=0 && tileMatrix[i-1][j]== null) {
		    			for(int p = j-1; p>= 0; p--)
		    				for(int r = j+1; r < 12; r++) {
		    					if(tileMatrix[i][p]== null && tileMatrix[i][r]== null) {
		    						if(tileMatrix[i][j] != null) {
		    						Score += tileMatrix[i][j].getNumber();
				    				tileMatrix[i][j] = null;
				    				break;
