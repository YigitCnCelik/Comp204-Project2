...
public class Tetromino {
   ...
   public void rotate() {
	   int n = tileMatrix.length; // n = number of rows = number of columns
	   Tile[][] rotatedTileMatrix = new Tile[n][n]; // create a new matrix to hold rotated tiles
	   // for each row
	   for (int row = 0; row < n; row++) {
	      // for each column
	      for (int col = 0; col < n; col++) {
	         // compute the new row and column indexes for the rotated tile
	         int newRow = col; // new row index is the current column index
	         int newCol = n - 1 - row; // new column index is (n - 1) minus the current row index
	         // get the tile at the current position
	         Tile tile = tileMatrix[row][col];
	         // set the tile at the new position in the rotated matrix
	         rotatedTileMatrix[newRow][newCol] = tile;
	      }
	   }
	   // update the tile matrix with the rotated matrix
